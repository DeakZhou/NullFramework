package com.ricky.f.util.lib.urlrouter;
...
public class UrlRouterApi {
    ...
    public static Route parseStartedRoute(Context context) {
        if (context != null && context instanceof Activity) {
            ...
            if (startedIntent.hasExtra(UrlRouterUtils.URL_ROUTER_REFERRER)) {
                return startedIntent.getParcelableExtra(UrlRouterUtils.URL_ROUTER_REFERRER);
            }
        }
        return null;
    }
    ...
    public static Route parseCurrentRoute(Context context) {
        if (context != null && context instanceof Activity) {
            ...
            if (uri == null)
                return null;
            route.scheme = UrlRouterApi.getScheme(uri);
            route.host = UrlRouterApi.getHost(uri);
            route.path = UrlRouterApi.getPath(uri);
            ResolveInfo resolveInfo = UrlRouterApi.queryActivity(context, startedIntent);
            if (resolveInfo == null)
                return route;
